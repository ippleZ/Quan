...
0⃣ 在 ⟦订阅链接⟧ 后加 "#" 使用, 不同参数用 "&" 连接 
 "你的订阅连接#emoji1&tfo1&in香港台湾"
 本地资源片段引用, 请将参数如 "#inxxx&outyyy" 填入资源片段的第 ① 行
 支持中文, "操作" 以下特殊字符时请先替换(URL-Encode) 
  """%2B", 空格"%20", "@""%40", "&""%26", ".""\.", ",""%2C"

1⃣ ⟦𝐬𝐞𝐫𝐯𝐞𝐫 节点⟧ 参数说明:
 emoji1(国行设备用2)/-1, 添加/删除节点名内地区旗帜;
 udp1/-1, tfo1/-1, 分别强制开启(关闭) 𝐮𝐝𝐩-𝐫𝐞𝐥𝐚𝐲/𝐟𝐚𝐬𝐭-𝐨𝐩𝐞𝐧;
 uot1, 开启 udp-over-tcptrue选项（仅限SS(R)）
 cert1/-1, 分别开启/关闭 𝐭𝐥𝐬 证书验证(默认关闭);
 ...
 in, out, regex, regout 分别为 保留、删除、正则保留、正则删除 节点;
  ...
  示范: "in香港.0\.2倍率台湾&outBGP&regexiplc"
 rename 重命名, "旧名@新名", "前缀@", "@后缀", 用 "" 连接多个参数;
  删除字段: "字段1.字段2", 想删除 "." 时用 "\." 替代
  示范: "rename香港@𝐇𝐊[𝐒𝐒]@@[1𝐗]流量.0\.2"
  默认 emoji 先生效, 如想调换顺序, 请用 rrname 参数
 replace 正则替换节点中字段, 可用于重命名/更改加密方式等
  replaceregex1@𝘀𝘁𝗿1regex2@𝘀𝘁𝗿2
 ptn/npt1-8, 将节点名英文/数字替换成样式 //𝐀/𝗮/𝔸/𝕒/ᵃ/ᴬ, ①\❶\⓵\𝟙\¹\₁\𝟏\𝟷
 delreg, 利用正则表达式来删除 "节点名" 中的字段( 慎用)
 aead-1, 关闭 Vmess 的 AEAD 参数
 hostxxx, 修改已有 host , 如要增加host，请用结尾
 obfsvhttp/shttp, 指定 obfsshadowsocks-http 或 obfsvmess-http 的特殊需求
 tsession0/1/2, 0/1 代表关闭 tls-session-ticket/reuse，2 表示全部关闭
 checkurlxxx , 指定 server_check_url 参数
 sort1/-1/x/参数规则, 按节点名 正/逆/随机/参数规则 排序
  参数规则是正则表达式或简单关键词, 用"" 或 "" 连接
  sort , 靠前排序
  sortIEPLIPLCBGP , 靠后排序
 info1, 开启通知提示机场 流量信息(如有提供);
 flow2022-06-02:1000:54, 订阅到期时间:总流量:已用流量
...
2⃣ ⟦𝐫𝐞𝐰𝐫𝐢𝐭𝐞 重写⟧/⟦𝐟𝐢𝐥𝐭𝐞𝐫 分流⟧ 参数说明:
 in, out, 根据关键词 保留/禁用 相关分流、重写规则;
 inhn, outhn, “保留/删除”主机名(𝒉𝒐𝒔𝒕𝒏𝒂𝒎𝒆);
  示范: 禁用 "淘宝比价" 及 "weibo" 的 js 同主机名
 𝐡𝐭𝐭𝐩𝐬://𝐦𝐲𝐥𝐢𝐬𝐭#outtb_price.jswb_ad.js&outhnweibo
 regex/regout, 正则保留/删除, 请自行折腾正则表达式;
  可与 in(hn)/out(hn) 一起使用，in(hn)/out(hn) 会优先执行;
  对 𝒉𝒐𝒔𝒕𝒏𝒂𝒎𝒆 & 𝐫𝐞𝐰𝐫𝐢𝐭𝐞/𝐟𝐢𝐥𝐭𝐞𝐫 同时生效( 慎用)
 policy 参数, 用于直接指定策略组，或为 𝐒𝐮𝐫𝐠𝐞 类型 𝗿𝘂𝗹𝗲-𝘀𝗲𝘁 生成策略组(默认"𝐒𝐡𝐚𝐰𝐧"策略组);
 psetregex1@policy1regex2@policy2, 为同一分流规则中不同关键词(允许正则表达式)指定不同策略组;
 replace 参数, 正则替换 𝐟𝐢𝐥𝐭𝐞𝐫/𝐫𝐞𝐰𝐫𝐢𝐭𝐞 内容, regex@newregex;
  将淘宝比价中脚本替换成 lite 版本(如有此版本的脚本)
   replace(price)(.*)@1_lite2
 dstrewrite/filter，分别为将 𝐦𝐨𝐝𝐮𝐥𝐞&𝗿𝘂𝗹𝗲-𝘀𝗲𝘁 转换成 重写/分流;
  默认将 𝐦𝐨𝐝𝐮𝐥𝐞 转换到重写, 𝗿𝘂𝗹𝗲-𝘀𝗲𝘁 转成分流
  把 𝗿𝘂𝗹𝗲-𝘀𝗲𝘁 中 url-regex 转成重写时, 必须要加 dstrewrite;
  把 𝐦𝐨𝐝𝐮𝐥𝐞 中的分流规则转换时, 必须要加 dstfilter
 cdn1, 将 github 脚本的地址转换成免翻墙 fastly.jsdelivr.net/gh
 fcr1/2/3, 为分流规则添加 force-cellular/multi-interface/multi-interface-balance 参数，强制移动数据/混合数据/负载均衡
 via接口, 为分流规则添加 via-interface 参数, 0 表示 via-interface%TUN%
 relay目标策略名, 批量将节点订阅转换为ip/host规则，用于实现代理链

3⃣ 其他参数
 通知参数 ntf0/1, 用于 关闭/打开 资源解析器的提示通知
  𝗿𝗲𝘄𝗿𝗶𝘁𝗲/𝗳𝗶𝗹𝘁𝗲𝗿 默认“开启”通知提示, 以防规则误删除
  𝘀𝗲𝗿𝘃𝗲𝗿 资源解析则默认”关闭“通知提示
 类型参数 typedomain-set/rule/module/list/nodes
  当解析器未能正确识别类型时, 可尝试使用此参数强制指定
 隐藏参数 hide0, 禁用筛除的分流/重写，默认方式为删除
 profile111 , URL-Scheme 添加 QuanX 类型配置中远程资源
----------------------------------------------------------
*/

/**
* 使用说明，
0⃣ 在QuantumultX 配置文件中[general] 部分，加入 
...
function Parser() {
 ...
 if (type0 ! "web" && type0 ! "wrong-field" && type0 ! "JS-0"){
  ...
  if (version 670 && typec!"") { //尝试跳转到正确类型
   RLink0[Field[typec]].push(resource.link", opt-parsertrue, tag下次添加资源长点8⃣") // 跳转URI-Scheme
...
function ParseUnknown(cnt){
 try {
  cnt JSON.parse(cnt)
  if(cnt) {
   notify(" 链接返回内容并非有效订阅" "⟦" subtag "⟧","⁉ 请自行检查原始链接，返回内容 ",JSON.stringify(cnt), bug_link)
  }
 } catch(err) {
...
function RegCheck(total, typen, paraname,regpara) {
 if(total.length 0){ 
  notify("‼ " typen "  " "⟦" subtag "⟧", " 筛选正则: " paraname "" regpara, " 筛选后剩余项为 0⃣ , 请检查正则参数及原始链接", nan_link)
 }else if((typen ! "节点订阅" && Pntf0 !0) (typen "节点订阅" && Pntf0 1)){
  var nolist total.length 10 ? emojino[total.length] : total.length
  notify(" " typen "  " "⟦" subtag "⟧", " 筛选正则: " paraname "" regpara, " 筛选后剩余以下" nolist "个匹配项 \n " total.join("\n "), sub_link)
 }
}
//判断订阅类型
function Type_Check(subs) {
  ...
  if ( (subs.indexOf(html) ! -1 subs.indexOf("doctype html") ! -1) && link0.indexOf("github.com" -1)) {
   ...
   Perror 1 ; // 无需反馈
   notify(" 你导入的链接内容为 JS 脚本"," 脚本内未有重写规则，无法解析使用", " 请不要跑来解析器反馈 \n"link0)
   type "JS-0"
...
function TagCheck_QX(content) {
 ...
 console.log(content)
  ...
  if (nulllist.length 1) {
    ...
    notify(" 引用" "⟦" subtag "⟧" " 内有" no "个空节点名 ", " 已将节点“类型IP”设为节点名", " " nulllist.join("\n "), nan_link)
  }
  if (duplist.length 1) {
    no duplist.length 10 ? emojino[duplist.length] : duplist.length;
   if (Pdel!1 && Pntf0 ! 0){
    notify(" 引用" "⟦" subtag "⟧" " 内有" no "个名字重复的节点 ", " 已添加数字区分, 删除请添加参数 del1:", " " duplist.join("\n "), nan_link)
   } else if (Pdel 1 && Pntf0 ! 0) {
    notify(" 引用" "⟦" subtag "⟧" " 内有" no "个名字重复的节点 ", " 已全部删除，如需保留请去除参数 del1:", " " duplist.join("\n "), nan_link)
   }
  }
  return Nlist
}
...
function HOST_Handle(cnt,phost) {
 phost"host"phost","
 if (phost.indexOf("") -1) { //只替换已有host类型
...
function Rewrite_Filter(subs, Pin, Pout,Preg,Pregout) {
  ...
  if (Pntf0 ! 0) {
    ...
    if (Pin0 && no1write ! " 0⃣ ") { //有 in 参数就通知保留项目
     if (Pout!0) {
  ...
  if (Nlist.length 0 ) { 
   if ((Pin0 Pout0 Phin0 Phout0 Pregout Preg)) {
    notify(" " "重写引用  " "⟦" subtag "⟧", " 筛选参数: " pfi pfo, " 筛选后剩余rewrite规则数为 0⃣ 条, 请检查参数及原始链接", nan_link) 
   } else {
    notify(" " "重写引用  " "⟦" subtag "⟧", " 解析后 rewrite 规则数为 0⃣ 条 " , " 请检查参数及原始链接内容", nan_link) 
   }
  }
...
function HostNamecheck(content, parain, paraout) {
  ...
  if (nname.length 0) {
    notify(" " "重写引用  " "⟦" subtag "⟧", " 筛选参数: " pfihn pfohn, " 主机名 hostname 中剩余 0⃣ 项, 请检查参数及原始链接", nan_link)
  }
  ...
  if (Tout ! "" && Tout ! null) { // 有 out 参数时
    ...
    if (Tin ! "" && Tin ! null) { //有 in 跟 out 参数时
      if (nlist.length 0) {
        ...
        notify(" " "分流引用  " "⟦" subtag "⟧", " 保留:" Tin ", 禁用: " Tout, " 筛选后剩余规则数为 0⃣ 条, 请检查参数及原始链接", nan_link)
      }
    } else {// if Tin (No Tin)
      if (nlist.length 0) {
        notify(" " "分流引用  " "⟦" subtag "⟧", " 禁用: " Tout, " 筛选后剩余规则数为 0⃣ 条, 请检查参数及原始链接", nan_link)
      }
    }
   nlist Phide 1? nlist : [...dlist,...nlist]
    ...
    } else { notify(" " "分流引用  " "⟦" subtag "⟧", " 保留:" Tin, " 筛选后剩余规则数为 0⃣ 条, 请检查参数及原始链接", nan_link) }
   nlist Phide 1? nlist : [...dlist,...nlist]
   //return nlist;
  } else { //if Tin
      ...
      try {
       if (Pdbg) {notify(i, type, list0[i])}
        if (type "vmess" && (list0[i].indexOf("remark") -1 && list0[i].indexOf("remarks") -1) && !/(obfsalterIdtype\@)\/.test(list0[i])) {
         ...
         Perror 1 ; // 无需反馈
         notify(" 你的 Quantumult X 版本暂未支持 Vless 节点","请 不要 跑来 解析器 反馈",list0[i])
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
 ...
 if (obfs "none") { //
  ...
  Perror 1 // 不需要反馈的类型
  if (Pntf0!0) {
  notify( " Quantumult X 暂不支持该类型节点", "已忽略以下 grpch2 vmess 节点",subs)
 }
  pdrop 1
 }
 if (obfs.indexOf("obfsover-tls") ! -1 obfs.indexOf("obfswss") ! -1) {
...
function Filter(servers, Pin, Pout) {
  ...
  if (Pntf0 1 && Delist.length 1) {//通知部分
    ...
    notify("‼ ⟦" subtag "⟧" "筛选后节点数为0⃣", " 请自行检查原始链接以及筛选参数", link0, sub_link);
  }
  return Nlist
}
...
function get_emoji(emojip, sname) {
  var Lmoji { 
  "‍": ["流量", "套餐", "剩余", "重置", "到期" , "时间", "应急", "过期", "Bandwidth", "expire", "Traffic", "traffic"],
  "": ["阿曼", " OM "],
  "": ["安道尔","安道爾", "Andorra"],
  "": ["安哥拉"],
  "": ["阿富汗"],
