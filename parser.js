...
if (version == 0) {
	$notify("请更新 Quantumult X 至最新商店版本\n", "当前版本可能无法正常使用部分功能", "\n点击跳转商店链接更新", update_link)
}
...
function ParseUnknown(cnt) {
	try {
		cnt = JSON.parse(cnt)
		if (cnt) {
			$notify("链接返回内容并非有效订阅" + "⟦" + subtag + "⟧", "⁉️ 请自行检查原始链接，返回内容 👇️👇️", JSON.stringify(cnt), bug_link)
		}
...
function ResourceParse() {
	...
	if (flag == 1) { //server 类型统一处理
		...
		if (total.length > 0) {
			...
			if (Perror == 0) {
				$notify("❓❓ 友情提示 ➟ " + "⟦" + subtag + "⟧", "⚠️解析后无有效内容", "🚥🚥 请自行检查相关参数, 或者点击通知跳转并发送链接反馈", bug_link)
			}
			total = errornode
			$done({
				content: errornode
...
function Type_Check(subs) {
	...
	if ((subs.indexOf(html) != -1 || subs.indexOf("doctype html") != -1) && link0.indexOf("github.com" == -1)) {
		...
		Perror = 1; // 无需反馈
		$notify("你导入的链接内容为 JS 脚本", "🚥 脚本内未有重写规则，无法解析使用", " 请⚠️不要⚠️跑来解析器🤖️反馈 \n" + link0)
		type = "JS-0"
...
function ToRaw(cnt) {
	...
	if (cnt) {
		...
		$notify("⚠️解析该资源" + " ⟦" + subtag + "⟧ 失败", "🚥 你的链接似乎是目录，而不是文件", "你的链接：" + link0, {
			"open-url": link0
		})
	}
	return cnt
...
function Rule_Handle(subs, Pout, Pin) {
	...
	if (Tout != "" && Tout != null) { // 有 out 参数时
		...
		if (dlist.length > 0) {
			...
			$notify("🤖 " + "分流引用  ➟ " + "⟦" + subtag + "⟧", "⛔️ 禁用: " + Tout, "未发现任何匹配项, 请检查参数或原始链接", nan_link)
		}
		...
		if (nlist.length > 0) {
			...
			$notify("🤖 " + "分流引用  ➟ " + "⟦" + subtag + "⟧", "✅ 保留:" + Tin, "筛选后剩余规则数为 0️⃣ 条, 请检查参数及原始链接", nan_link)
		}
		nlist = Phide == 1 ? nlist : [...dlist, ...nlist]
		//return nlist;
	} else { //if Tin
		...
		if (list0[i].trim().length > 3 && !/\;|\/|\#/.test(list0[i][0]) && list0[i].indexOf(" url ") == -1) {
			...
			try {
				...
				if (type == "vmess" && (list0[i].indexOf("remark=") == -1 && list0[i].indexOf("remarks=") == -1) && !/(obfs|alterId|type)\=/.test(list0[i])) {
					...
					Perror = 1; // 无需反馈
					$notify("Quantumult X 暂未支持 Vless 类型节点", "请 ⚠️不要跑来 解析器🤖️ 反馈", list0[i])
				}
			} catch (e) {
				failedList.push(` << < \nContent: $ {
						list0[i]
...
function get_emoji(emojip, sname) {
	var Lmoji = {
		...
		"🇭🇷": ["克罗地亚", "HR", "克罗地亚", "Croatia"],
		"🇨🇦": ["CA", "Canada", "CANADA", "CAN", "Waterloo", "加拿大", "蒙特利尔", "温哥华", "枫叶", "枫叶", "滑铁卢", "多伦多"],
		"🇨🇭": ["瑞士", "苏黎世", "Switzerland", "CH "],
		"🇳🇬": ["尼日利亚", "NG", "尼日利亚", "拉各斯", "Nigeria"],
		"🇨🇿": ["Czechia", "捷克"],
		"🇸🇰": ["斯洛伐克", "SK", "Slovakia"],
		"🇸🇮": ["斯洛文尼亚", "斯洛文尼亚", "Slovenia"],
		"🇦🇲": ["亚美尼亚", "亚美尼亚", "Armenia"],
		"🇷🇸": ["RS ", "RS_", "塞尔维亚", "塞尔维亚", "Seville", "Sevilla"],
		"🇲🇩": ["摩尔多瓦", " MD-", "摩尔多瓦", "Moldova", " MD "],
		"🇩🇪": ["DE ", "DE-", "DE_", "German", "GERMAN", "德国", "德国", "法兰克福", "京德", "沪德", "广德", "沪德", "广德"],
		"🇩🇰": ["DK", "DNK", "丹麦", "丹麦", "Denmark"],
		"🇪🇸": ["ES", "西班牙", "Spain"],
		...
		"🇲🇴": ["MO", "Macao", "Macau", "MAC", "澳门", "澳门", "CTM"],
		"🇰🇿": ["哈萨克斯坦", "哈萨克斯坦", "Kazakhstan"],
		"🇱🇦": ["老挝", "老挝", "Laos"],
		"🇭🇺": ["匈牙利", "Hungary"],
		"🇱🇹": ["立陶宛", "Lithuania"],
		"🇱🇰": ["斯里兰卡", "斯里兰卡", "Sri Lanka"],
		"🇧🇾": ["BY", "白俄罗斯", "白俄罗斯", "White Russia", "Republic of Belarus", "Belarus"],
		"🇷🇺": ["RU ", "RU-", "RU_", "RUS", "Russia", "俄罗斯", "毛子", "俄国", "俄罗斯", "伯力", "莫斯科", "圣彼得堡", "西伯利亚", "新西伯利亚", "京俄", "杭俄", "广俄", "沪俄", "广俄", "沪俄"],
		"🇸🇬": ["SG", "Singapore", "SINGAPORE", "新加坡", "狮城", "狮城", "沪新", "京新", "泉新", "穗新", "深新", "杭新", "广新", "广新", "沪新"],
		"🇺🇸": ["US", "USA", "America", "United States", "美国", "美", "京美", "波特兰", "达拉斯", "俄勒冈", "凤凰城", "费利蒙", "硅谷", "硅谷", "拉斯维加斯", "洛杉矶", "圣何塞", "圣荷西", "圣克拉拉", "西雅图", "芝加哥", "沪美", "哥伦布", "纽约"],
		"🇹🇼": ["TW", "Taiwan", "TAIWAN", "台湾", "台北", "台中", "新北", "彰化", "CHT", "台", "HINET"],
		"🇮🇩": ["ID ", "IDN ", "Indonesia", "印尼", "印度尼西亚", "雅加达"],
		"🇮🇪": ["Ireland", "IRELAND", "IE ", "爱尔兰", "爱尔兰", "都柏林"],
		"🇮🇱": ["Israel", "以色列"],
		"🇮🇳": ["India", "IND", "INDIA", "印度", "孟买", "Mumbai", "IN "],
		"🇮🇸": ["IS", "ISL", "冰岛", "冰岛", "Iceland"],
		"🇰🇵": ["KP", "朝鲜", "North Korea"],
		...
		"🇲🇾": [" MY", "Malaysia", "MALAYSIA", "马来西亚", "马来", "马来", "大马", "大马", "马来西亚", "吉隆坡"],
		"🇲🇲": ["缅甸"],
		...
		"🇧🇷": ["BR", "Brazil", "巴西", "圣保罗"],
		"🇯🇵": ["JP", "Japan", "JAPAN", "日本", "东京", "大坂", "埼玉", "京日", "苏日", "沪日", "上日", "穗日", "川日", "中日", "泉日", "杭日", "深日", "辽日", "广日", "Tokyo"],
		"🇦🇷": ["AR ", "Argentina", "阿根廷", "AR-"],
		"🇳🇴": ["Norway", "挪威", "NO"],
		"🇵🇱": [" PL", "POL", "波兰", "波兰", "Poland"],
		"🇨🇱": ["智利", "Chile", "CHILE"],
...
function Clash2QX(cnt) {
	...
	for (i = 0; i < nl; i++) {
		try {
			...
			$notify(`⚠️该节点解析错误, 暂时已忽略处理`, `可点击通知并发送链接反馈至bot`, JSON.stringify(node), bug_link)
			$notify(`⚠️错误内容如下`, `可复制错误内容到反馈bot`, JSON.stringify(node) + "\n\n" + e)
		}
	}
