let[link0, content0, subinfo] = [$resource.link, $resource.content, $resource.info] let version = typeof $environment != "undefined" ? Number($environment.version.split("build")[1]) : 0 let Perror = 0 const subtag = typeof $resource.tag != "undefined" ? $resource.tag : "";
...
var para1 = para.slice(para.indexOf("#") + 1).replace(/\$type/g, "node_type_para_prefix").replace(/\$emoji/g, "node_emoji_flag_prefix").replace(/\$tag/g, "node_tag_prefix").replace(/\$index/g, "node_index_prefix") var mark0 = para.indexOf("#") != -1 ? true : false;
var Pinfo = mark0 && para1.indexOf("info=") != -1 ? para1.split("info=")[1].split("&")[0] : 0;
var ntf_flow = 0;
const Base64 = new Base64Code();
const escapeRegExp = str = > str.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, '\\$&');
var link1 = link0.split("#")[0] const qxpng = "https://raw.githubusercontent.com/crossutility/Quantumult-X/master/quantumult-x.png"
const subinfo_link = {
...
const ADDRes = `quantumult - x: var RLink0 = {
	"filter_remote": [],
	"rewrite_remote": [],
	"server_remote": [],
}
...
SubFlow() var Pin0 = mark0 && para1.indexOf("in=") != -1 ? (para1.split("in=")[1].split("&")[0].split("+")).map(decodeURIComponent) : null;
...
var Phin0 = mark0 && para1.indexOf("inhn=") != -1 ? (para1.split("inhn=")[1].split("&")[0].split("+")).map(decodeURIComponent) : null;
var Phout0 = mark0 && para1.indexOf("outhn=") != -1 ? (para1.split("outhn=")[1].split("&")[0].split("+")).map(decodeURIComponent) : null;
var Preplace = mark0 && para1.indexOf("replace=") != -1 ? para1.split("replace=")[1].split("&")[0] : null;
...
var Psrename = mark0 && para1.indexOf("srename=") != -1 ? Base64.decode(para1.split("srename=")[1].split("&")[0]) : null;
...
var Pptn = mark0 && para1.indexOf("ptn=") != -1 ? para1.split("ptn=")[1].split("&")[0] : "";
var Pnptn = mark0 && para1.indexOf("npt=") != -1 ? para1.split("npt=")[1].split("&")[0] : "";
...
var Pfcr = mark0 && para1.indexOf("fcr=") != -1 ? para1.split("fcr=")[1].split("&")[0] : "";
var Pvia = mark0 && para1.indexOf("via=") != -1 ? para1.split("via=")[1].split("&")[0] : "";
var Paead = mark0 && para1.indexOf("aead=") != -1 ? para1.split("aead=")[1].split("&")[0] : "";
var Phost = mark0 && (para.indexOf("#host=") != -1 || para.indexOf("&host=") != -1) ? (para.indexOf("#host=") != -1 ? para.split("#host=") : para.split("&host="))[1].split("&")[0] : "";
var Pcsha256 = mark0 && para1.indexOf("csha=") != -1 && version >= 646 ? para1.split("csha=")[1].split("&")[0] : "";
var Ppsha256 = mark0 && para1.indexOf("psha=") != -1 && version >= 646 ? para1.split("psha=")[1].split("&")[0] : "";
var typeQ = $resource.type ? $resource.type : "unsupported"
var PRelay = mark0 && para1.indexOf("relay=") != -1 ? decodeURIComponent(para1.split("relay=")[1].split("&")[0]) : "";
var PUOT = mark0 && para1.indexOf("uot=") != -1 && version >= 665 ? para1.split("uot=")[1].split("&")[0] : "";
var PcheckU = mark0 && para1.indexOf("checkurl=") != -1 ? decodeURIComponent(para1.split("checkurl=")[1].split("&")[0]) : "";
typeQ = PRelay != "" ? "server" : typeQ
var typec = ""
var Pflow = mark0 && para1.indexOf("flow=") != -1 ? para1.split("flow=")[1].split("&")[0] : 0;
var PProfile = mark0 && para1.indexOf("profile=") != -1 ? para1.split("profile=")[1].split("&")[0] : 0;
var Palpn = mark0 && para1.indexOf("alpn=") != -1 && version >= 712 ? para1.split("alpn=")[1].split("&")[0] : "";
var Pobfs = mark0 && para1.indexOf("obfs=") != -1 && version >= 770 ? para1.split("obfs=")[1].split("&")[0] : "";
var Psession = mark0 && para1.indexOf("tsession=") != -1 && version >= 771 ? para1.split("tsession=")[1].split("&")[0] : "";
var RegoutList = [];
var ADDres = `quantumult - x: var RLink = ` {
	"server_remote": [sremoteposition],
	"filter_remote": [fremoteposition],
	"rewrite_remote": [rremoteposition]
}`
var ProfileInfo = {
	"server": "",
	"filter": "",
	...
	function Profile_Handle() {
		let a = content0 PProfile = PProfile == 1 ? "001" : PProfile PProfile = PProfile == 8 ? "010" : PProfile PProfile = PProfile == 9 ? "011" : PProfile srm = a.split("[server_remote]")[1] && String(PProfile)[0] == "1" ? VCheck(a.split("[server_remote]")[1].split("[")[0]) : ""
		...
		RLink = RLink.replace("sremoteposition", srm).replace("fremoteposition", frm).replace("rremoteposition", rrm) ADDres = ADDres.replace("url-encoded-json", encodeURIComponent(RLink))
	}
var Finfo = {}
if (Pflow != 0) {
	Pflow = Pflow.split(":") var Bdate = Date.parse(new Date(Pflow[0])) / 1000
	...
	function Parser() {
		...
		if (type0 != "web" && type0 != "wrong-field" && type0 != "JS-0") {
			...
			if (version >= 670 && typec != "") {
				RLink0[Field[typec]].push($resource.link + ", opt-parser=true, tag=下次添加资源🉑️长点❤️8⃣️") var flink = ADDRes.replace(/url-encoded-json/, encodeURIComponent(JSON.stringify(RLink0))) const bug_linkx = {
					"open-url": flink,
					"media-url": "https://shrtm.nu/obcB"
				}
...
if (typeof($resource) !== "undefined" && PProfile == 0) {
	...
	$notify("⚠️请忽略报错提示, 点击此通知跳转", "添加配置中的有效远程资源👇 [" + PProfile + "]", ADDres, openlink) total = ProfileInfo[typeQ] $done({
		content: total
	})
}
...
function ResourceParse() {
	if (type0 == "Subs-B64Encode") {
		...
		flag = 2 total = Rewrite_Filter(isQuanXRewrite(content0.split("\n")), Pin0, Pout0, Preg, Pregout);
		if (Preplace) {
		...
		if (Preg && total.length != 0) {
			total = total.map(Regex).filter(Boolean) RegCheck(total, "分流引用", "regex", Preg)
		}
		if (Pregout && total.length != 0) {
			total = total.map(RegexOut).filter(Boolean) RegCheck(total, "分流引用", "regout", Pregout)
		}
		if (Preplace) {
			total = ReplaceReg(total, Preplace)
		}
		...
		total = total.length < 100 ? total.filter((ele, pos) = > total.indexOf(ele) == pos) : total total = total.join("\n")
	} else if (content0.trim() == "") {
		$notify("‼️ 引用" + "⟦" + subtag + "⟧" + " 返回內容为空", "⁉️ 点通知跳转以确认链接是否失效", para.split("#")[0], nan_link);
		flag = 0
	} else if (type0 == "sub-http") {
		let url = VCheck(String(Base64.decode(content0.split("sub://")[1].split("#")[0]) + ", opt-parser=true, tag=" + (new Date()).getTime())) RLink = RLink.replace("sremoteposition", url).replace("fremoteposition", "").replace("rremoteposition", "") let ADDres0 = ADDres.replace("url-encoded-json", encodeURIComponent(RLink)) openlink = {
			"open-url": ADDres0
		}
		$notify("⚠️ 该链接为节点订阅, 请点击此通知跳转添加", url, ADDres0, openlink) flag = -1 total = ""
	} else if (type0 == "unknown") {
		ParseUnknown(content0) flag = -1
	} else if (type0 == "profile") {
		PProfile = "111"
		Profile_Handle() openlink = {
			"open-url": ADDres
		}
		$notify("⚠️ 该链接为完整配置文件, 请点击此通知跳转", "添加配置中的有效远程资源👇 [" + PProfile + "]", ADDres, openlink) flag = -1;
		total = ""
	} else if (type0 == "JS-0") {
		total = ""
	}
	if (flag == 1) {
		...
		if (Preg) {
			total = total.map(Regex).filter(Boolean) RegCheck(total, "节点订阅", "regex", Preg)
		}
		if (Pregout) {
			total = total.map(RegexOut).filter(Boolean) RegCheck(total, "节点订阅", "regout", Pregout)
		}
		if (Psfilter) {
		...
		if (Pregdel) {
			delreg = Pregdel total = total.map(DelReg)
		}
		if (Psrename) {
		...
		if (total.length > 0) {
			...
			total = total.map(type_handle).map(emoji_prefix_handle).map(tag_handle) if (Psort0) {
				total = QXSort(total, Psort0)
			}
			total = para1.indexOf("node_index_prefix") != -1 ? index_handle(total) : total total = TagCheck_QX(total).join("\n") if (PUOT == 1) {
				total = total.split("\n").map(UOT).join("\n")
			}
			if (Pcnt == 1) {
...
function flowcheck(cnt) {
	for (var i = 0; i < cnt.length; i++) {
		var item = cnt[i];
		var nl = item.slice(item.indexOf("tag")) var nm = nl.slice(nl.indexOf("=") + 1) if (item.indexOf("剩余流量") != -1) {
			flow = nm
...
function RegCheck(total, typen, paraname, regpara) {
	if (total.length == 0) {
		...
		var nolist = total.length <= 10 ? emojino[total.length] : total.length $notify("🤖 " + typen + "  ➟ " + "⟦" + subtag + "⟧", "⛔️ 筛选正则: " + paraname + "=" + regpara, "⚠️ 筛选后剩余以下" + nolist + "个匹配项 \n ⨷ " + total.join("\n ⨷ "), sub_link)
	}
}
...
function Type_Check(subs) {
	...
	var RewriteK = [" url 302", " url 307", " url reject", " url script", " url req", " url res", " url echo", " url-and-header 302", " url-and-header 307", " url-and-header reject", " url-and-header script", " url-and-header req", " url-and-header res", " url-and-header echo"]
	...
	var subi = subs.replace(/ /g, "") const RuleCheck = (item) = > subi.toLowerCase().indexOf(item) != -1;
	const NodeCheck = (item) = > subi.toLowerCase().indexOf(item.toLowerCase()) != -1;
	const NodeCheck1 = (item) = > subi.toLowerCase().indexOf(item.toLowerCase()) != -1;
	const NodeCheck2 = (item) = > subi.toLowerCase().indexOf(item.toLowerCase()) != -1;
	...
	var subsn = subs.split("\n") if ((subs.indexOf(html) != -1 || subs.indexOf("doctype html") != -1) && link0.indexOf("github.com" == -1)) {
		...
		typec = "rewrite"
		type = (typeQ == "unsupported" || typeQ == "rewrite") ? "rewrite" : "wrong-field"
		...
		typec = "server-surge"
		type = (typeQ == "unsupported" || typeQ == "server" || typeQ == "uri") ? "Surge" : "wrong-field";
...
function TagCheck_QX(content) {
	...
	console.log(content) var Olist = content.map(item = > item.trim()) var [Nlist, nmlist] = [
		[],
		[]
	]
	...
	for (var i = 0; i < Olist.length; i++) {
		...
		if (item.replace(/ /gm, "").indexOf("tag=") != -1) {
			var nl = item.slice(item.indexOf("tag")) var nm = nl.slice(nl.indexOf("=") + 1) if (nm == "") {
				tp = typefix[item.split("=")[0].trim()][3] nm = tp + " | " + item.split("=")[1].split(",")[0].split(":")[0] item = item.split("tag")[0] + "tag=" + nm.replace("shadowsocks", "ss") nulllist.push(nm.replace("shadowsocks", "ss"))
			}
			var ni = 0
			while (nmlist.indexOf(nm) != -1) {
				nm = ni == 0 ? nm + NoReplace(ni + 1) : nm.split(" ").slice(0, nm.split(" ").length - 2).join(" ") + NoReplace(ni + 1) item = Pdel != 1 ? item.split("tag")[0] + "tag=" + nm : ""
				ni = ni + 1
			}
			if (ni != 0) {
				duplist.push(nm)
			}
			nmlist.push(nm) if (Pcap) {
				item = Capitalize(item, Pcap) console.log(item)
			}
			if (Pptn || Pnptn) {
				item = Pattern(item, Pptn, Pnptn) console.log(item)
			}
			ni = 0
			if (item) {
				Nlist.push(item)
...
function PatternN(cnt, para, npara) {
	if (cnt) {
		if (para != "") {
			for (var i = 0; i < 26; i++) {
				cnt = cnt.toLowerCase() cnt = cnt.replace(new RegExp(pat[0][i], "gmi"), pat[para][i])
			}
		}
		if (npara != "") {
			for (var i = 0; i < 10; i++) {
...
function getnode_type(item, ind) {
	if (item.trim() != "" && item.indexOf("tag=") != -1) {
		ind = !/^(0|1|2|3|4|5|6|7)$/.test(ind) ? 8 : ind typefix = {
		...
		typefix["shadowsocks"] = item.indexOf("ssr-protocol") != -1 ? typefix["shadowsocksr"] : typefix["shadowsocks"] tp = typefix[item.split("=")[0].trim()][ind]
		return tp
	}
}
...
function index_handle(item) {
	items = item.map(item = > item.trim()).filter(Boolean) let b = Array.from(new Array(items.length), (val, index) = > index + 1);
	for (var i = 0; i < items.length; i++) {
		if (items[i].indexOf("node_index_prefix") != -1) {
			ind = items[i].split("node_index_prefix")[1][0] ind = !/^(0|1|2|3|4|5|6|7|8)$/.test(ind) ? 0 : ind console.log("handle index" + ind) items[i] = items[i].replace(/node_index_prefix(\d{0,1})/g, PatternN((i + 1).toString(), "", ind))
		}
	}
...
function ServerRelay(src, dst) {
	...
	for (var i = 0; i < src.length; i++) {
		serverA = src[i].indexOf("-host") == -1 ? src[i].split("=")[1].split(":")[0].trim() : src[i].split("-host")[1].split("=")[1].split(",")[0].trim() type = /^[a-z]/.test(serverA) || /[a-z]$/.test(serverA) ? "host" : "ip"
		rst = type == "ip" ? "ip-cidr," + serverA + "/32," + dst : "host-suffix," + serverA + "," + dst rsts.push(rst)
	}
	return rsts.join("\n")
}

function rawtest(cnt) {
	var Preg0 = RegExp(".*js-file-line\".*?\<\/td\>", "i") if (Preg0.test(cnt)) {
		return cnt.replace(/(.*js-file-line\"\>)(.*?)(\<\/td\>)/g, "$2").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").trim()
	}
}

function ToRaw(cnt) {
	cnt = cnt.split("\n").map(rawtest).filter(Boolean).join("\n") var rawlink = link0.replace("github.com", "raw.githubusercontent.com").replace("/blob", "") if (cnt) {
...
function CDN(cnt) {
	console.log("CDN start") cnt = cnt.join("\n").replace(/https:\/\/raw.githubusercontent.com\/(.*?)\/(.*?)\/(.*)/gmi, "https://fastly.jsdelivr.net/gh/$1/$2@$3") return cnt
}
...
function SHA256_Handle(cnt, pcsha256, ppsha256) {
	if (cnt.indexOf("over-tls=true") != -1 || cnt.indexOf("obfs=wss") != -1) {
		cnt = cnt.replace(/tls-verification\s*\=\s*false(\s*)\,/, "") cnt = pcsha256 != "" ? cnt.replace(/tag\s*\=/, "tls-cert-sha256=" + pcsha256 + ", tag=") : cnt cnt = ppsha256 != "" ? cnt.replace(/tag\s*\=/, "tls-pubkey-sha256=" + ppsha256 + ", tag=") : cnt
	}
	return cnt
}

function ALPN_Handle(cnt, palpn) {
	cnti = cnt.replace(/\s/gmi, "") if (cnti.indexOf("obfs=over-tls") != -1 || cnti.indexOf("over-tls=true") != -1) {
		...
		if (!NoteK.some(notecheck)) {
			if (subs[i].slice(0, 9) == "URL-REGEX") {
				...
				if (!/header=".*content-type/i.test(subs[i]) && /blank/i.test(fn)) {
					...
					rw = subs[i].replace(/ /g, "").split("data=")[0].replace(/\"/g, "") + " url echo-response text/html echo-response " + subs[i].split("data=")[1].split(" ")[0].replace(/\"/g, "").replace(/ /g, "") if (subs[i].indexOf("header=") != -1) {
						if (subs[i].indexOf("Content-Type:") != -1) {
							let tpe = subs[i].split("header=")[1].split("Content-Type:")[1].split(",")[0].replace(/\"/g, "") rw = rw.replace(/text\/html/g, tpe)
						}
					}
				}
				nrw.push(rw)
...
function SCP2QX(subs) {
	var nrw = []
	var rw = ""
	var RewriteK = [" url 302", " url 307", " url reject", " url script", " url req", " url res", " url echo", " url-and-header 302", " url-and-header 307", " url-and-header reject", " url-and-header script", " url-and-header req", " url-and-header res", " url-and-header echo"] subs = subs.split("\n").map(x = > x.trim().replace(/\s+/g, " ")) for (var i = 0; i < subs.length; i++) {
		try {
			subs[i] = subs[i].replace("^http", "http") if (subs[i].slice(0, 8) == "hostname") {
				hn = subs[i].replace(/\%.*\%/g, "").replace(/\:\d*/g, "") nrw.push(hn)
			}
			var SC = ["type=", ".js", "pattern=", "script-path="]
			var NoteK = ["//", "#", ";"];
			const sccheck = (item) = > subs[i].indexOf(item) != -1 const notecheck = (item) = > subs[i].indexOf(item) == 0 const RewriteCheck = (item) = > subs[i].indexOf(item) != -1;
			if (!NoteK.some(notecheck) && !RewriteK.some(RewriteCheck)) {
				if (SC.every(sccheck)) {
					ptn = subs[i].replace(/\s/gi, "").split("pattern=")[1].split(",")[0] js = subs[i].replace(/\s/gi, "").split("script-path=")[1].split(",")[0] type = subs[i].replace(/\s/gi, "").split("type=")[1].split(",")[0].trim() subsi = subs[i].replace(/ /g, "").replace(/\=true/g, "=1") if (type == "http-response" && subsi.indexOf("requires-body=1") != -1) {
						...
						rw = ptn + " url " + type + js nrw.push(rw)
					}
				} else if (/\s30(7|2)$/.test(subs[i])) {
					rw = subs[i].split(" ")[0] + " url " + subs[i].split(" ")[2] + " " + subs[i].split(" ")[1].trim() nrw.push(rw)
				} else if (/\s\-\s30(2|7)\s/.test(subs[i])) {
					rw = subs[i].replace(" - ", " url ") nrw.push(rw)
				} else if (subs[i].split(" ")[2] == "header") {
					var pget = subs[i].split(" ")[0].split(".com")[1]
					var pgetn = subs[i].split(" ")[1].split(".com")[1] rw = subs[i].split(" ")[0] + " url 302 " + subs[i].split(" ")[1] nrw.push(rw)
				} else if (subs[i].split(" ")[1] == "header-replace") {
					console.log(subs[i]) var pget = subs[i].split("header-replace")[1].split(":")[0].trim() var pgetn = subs[i].split("header-replace")[1].trim() rw = subs[i].split(" ")[0] + " url request-header " + "(.+\\r\\n)" + pget + ":.+(\\r\\n) request-header " + "$1" + pgetn + "$2"
					nrw.push(rw)
				} else if (subs[i].indexOf(" _ reject") != -1) {
					rw = subs[i].split(" ")[0] + " url reject-200"
					nrw.push(rw)
				} else if (subs[i].indexOf(" - reject") != -1 || subs[i].indexOf(" - REJECT") != -1) {
					rw = subs[i].replace(" - ", " url ").toLowerCase() nrw.push(rw)
				} else if (subs[i].split(" ").length == 2 && /\s(reject)$/.test(subs[i])) {
					rw = subs[i].replace(" reject", " url reject") nrw.push(rw)
				} else if (subs[i].indexOf("script-path") != -1) {
					type = subs[i].replace(/\s+/g, " ").split(" ")[0] js = subs[i].split("script-path")[1].split("=")[1].split(",")[0] ptn = subs[i].replace(/\s+/g, " ").split(" ")[1] subsi = subs[i].replace(/ /g, "").replace(/\=true/g, "=1") if (type == "http-response" && subsi.indexOf("requires-body=1") != -1) {
						type = "script-response-body "
					} else if (type == "http-response" && subsi.indexOf("requires-body=1") == -1) {
						type = "script-response-header "
					} else if (type == "http-request" && subsi.indexOf("requires-body=1") != -1) {
...
function SGMD2QX(subs) {
	var nrw0 = URX2QX(subs) var nrw1 = SCP2QX(subs) var nrwt = [...nrw0, ...nrw1]
	return nrwt
}
...
function Rewrite_Filter(subs, Pin, Pout, Preg, Pregout) {
	...
	for (var i = 0; i < subs.length; i++) {
		subi = subs[i].trim();
		var subii = subi.replace(/ /g, "") if (subi != "" && (subi.indexOf(" url ") != -1 || subi.indexOf(" url-and-header ") != -1 || /^hostname\=/.test(subii))) {
			const notecheck = (item) = > subi.indexOf(item) == 0
			if (noteK.some(notecheck)) {
				continue
			} else if (hnc == 0 && subii.indexOf("hostname=") == 0) {
				hostname = (Phin0 || Phout0 || Preg || Pregout) ? HostNamecheck(subi, Phin0, Phout0) : subi
			} else if (subii.indexOf("hostname=") != 0) {
				var inflag = Rcheck(subi, Pin);
				var outflag = Rcheck(subi, Pout);
				if (outflag == 1 || inflag == 0) {
					dwrite.push(subi.replace(" url ", " - ").replace(" url-and-header ", " - "))
	...
	if (Pntf0 != 0) {
		nowrite = dwrite.length <= 10 ? emojino[dwrite.length] : dwrite.length no1write = Nlist.length <= 10 ? emojino[Nlist.length] : Nlist.length
	...
	if (Preg) {
		Nlist = Nlist.map(Regex).filter(Boolean) RegCheck(Nlist, "重写引用", "regex", Preg)
	}
	if (Pregout) {
		Nlist = Nlist.map(RegexOut).filter(Boolean) RegCheck(Nlist, "重写引用", "regout", Pregout)
	}
	if (hostname != "") {
		Nlist.push(hostname)
	}
...
function HostNamecheck(content, parain, paraout) {
	...
	for (var i = 0; i < hname.length; i++) {
		dd = hname[i] const excludehn = (item) = > dd.indexOf(item) != -1;
		if (paraout && paraout != "") {
	...
	if (Preg) {
		nname = nname.map(Regex).filter(Boolean) RegCheck(nname, "主机名hostname", "regex", Preg)
	}
	if (Pregout) {
		nname = nname.map(RegexOut).filter(Boolean) RegCheck(nname, "主机名hostname", "regout", Pregout)
	}
...
function Rcheck(content, param) {
	name = content.toUpperCase() if (param) {
		var flag = 0;
...
function Rule_Handle(subs, Pout, Pin) {
	...
	if (Tout != "" && Tout != null) {
		var dlist = [];
		for (var i = 0; i < cnt.length; i++) {
			cc = cnt[i].replace(/^\s*\-\s/g, "").replace(/\"|\'/g, "").trim() const exclude = (item) = > cc.indexOf(item) != -1;
		...
		var dlist = [];
		for (var i = 0; i < cnt.length; i++) {
			cc = cnt[i].replace(/^\s*\-\s/g, "").trim() const RuleCheck = (item) = > cc.indexOf(item) != -1;
			const CommentCheck = (item) = > cc.toLowerCase().indexOf(item) == 0;
	...
	nlist = Pfcr == 1 ? nlist.filter(Boolean).map(item = > item + ", force-cellular") : nlist.filter(Boolean) nlist = Pfcr == 2 ? nlist.filter(Boolean).map(item = > item + ", multi-interface") : nlist.filter(Boolean) nlist = Pfcr == 3 ? nlist.filter(Boolean).map(item = > item + ", multi-interface-balance") : nlist.filter(Boolean) if (Pvia != "") {
		nlist = Pvia == 0 ? nlist.filter(Boolean).map(item = > item + ", via-interface=%TUN%") : nlist.filter(Boolean).map(item = > item + ", via-interface=" + Pvia)
	}
	nlist = nlist.map(item = > item.replace(/:\d*\s*,/g, ",").replace(/(\'|\")/g, "")) return nlist
}
...
function Rule_Policy(content) {
	...
	if (RuleK1.some(RuleCheck1) && !RuleK.some(RuleCheck)) {
		if (cnt.length == 3 && cnt.indexOf("no-resolve") == -1) {
			ply0 = Ppolicy != "Shawn" ? Ppolicy : cnt[2] nn = cnt[0] + ", " + cnt[1] + ", " + ply0
...
function rule_list_handle(cnt) {
	var RuleK = ["//", "#", ";", "[", "!", "/"] const RuleCheck = (item) = > cnt.trim().indexOf(item) == 0;
	const nocheck = (item) = > /^\d+$/.test(item) cnt = cnt.split("#")[0].trim() if (cnt.trim().indexOf(" ") == -1 && cnt.trim() != "" && !RuleK.some(RuleCheck)) {
		if (cnt.indexOf("::") != -1 && cnt.indexOf("/") != -1) {
			cnt = "ip6-cidr, " + cnt cnt = Ppolicy == "Shawn" ? cnt + ", Shawn" : cnt + ", " + Ppolicy
		} else if (cnt.split("/").length == 2) {
			cnt = "ip-cidr, " + cnt cnt = Ppolicy == "Shawn" ? cnt + ", Shawn" : cnt + ", " + Ppolicy
			...
			if (!/^('|")/.test(cnt)) {
				if (!/\*|\+/.test(cnt[0])) {
					cnt = cnt[0] == "." ? cnt.replace(".", "") : cnt cnt = "host-suffix, " + cnt
				} else {
					cnt = "host-wildcard, " + cnt
				}
			} else {
				cnt = cnt.replace(/'|"/g, "").trim() if (/^\.|\*\./.test(cnt) || cnt.indexOf("*") != -1) {
...
function policy_sets(cnt, para) {
	pcnt = para.split("+") cnt = cnt
	for (i = 0; i < pcnt.length; i++) {
		console.log(pcnt[i]) if (pcnt[i].indexOf("@") != -1) {
			cnt = cnt.map(item = > filter_set(item, pcnt[i]))
		}
	}
	cnt = cnt.filter(Boolean) return cnt console.log(cnt)
}

function filter_set(cnt, para) {
	if (cnt) {
		paras = [para.split("@")[0], para.slice(para.split("@")[0].length + "@".length)] console.log(para.split("@")[0].length + "@".length, paras) cnt = cnt.split(",") reg = RegExp(paras[0]) console.log(paras, cnt) if (cnt.length == 3) {
...
function Subs2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
	...
	for (var i = 0; i < list0.length; i++) {
		...
		if (list0[i].trim().length > 3 && !/\;|\/|\#/.test(list0[i][0]) && list0[i].indexOf(" url ") == -1) {
			var type = list0[i].split("://")[0].trim() var listi = list0[i].replace(/ /g, "") var tag0 = list0[i].indexOf("tag=") != -1 ? list0[i].split(/\&*(emoji|udp|tfo|cert|rename|replace)\=/)[0].split("tag=")[1] : ""
			list0[i] = (type == "ssr") ? list0[i].split(/#|,|，/)[0] : list0[i] const NodeCheck = (item) = > listi.toLowerCase().indexOf(item) != -1;
			...
			try {
				...
				if (type == "vmess" && (list0[i].indexOf("remark=") == -1 && list0[i].indexOf("remarks=") == -1) && !/(obfs|alterId|type)\=/.test(list0[i])) {
					var bnode = Base64.decode(list0[i].split("vmess://")[1]) if (bnode.indexOf("over-tls=") == -1) {
					...
					node = VR2QX(list0[i], Pudp, Ptfo, Pcert0, PTls13) node = tag0 != "" ? URI_TAG(node, tag0) : node
				} else if (type == "socks" && list0[i].indexOf("remarks=") != -1) {
					node = S5R2QX(list0[i]) node = tag0 != "" ? URI_TAG(node, tag0) : node
				} else if (type == "ssocks" && list0[i].indexOf("remarks=") != -1) {
					node = S5R2QX(list0[i], tlsp = "over-tls") node = tag0 != "" ? URI_TAG(node, tag0) : node
				} else if (type == "ssr") {
					node = SSR2QX(list0[i], Pudp, Ptfo) node = tag0 != "" ? URI_TAG(node, tag0) : node
				} else if (type == "ss") {
					node = SS2QX(list0[i], Pudp, Ptfo) node = tag0 != "" ? URI_TAG(node, tag0) : node
				} else if (type == "ssd") {
					node = SSD2QX(list0[i], Pudp, Ptfo)
				} else if (type == "trojan") {
					node = TJ2QX(list0[i], Pudp, Ptfo, Pcert0, PTls13) node = tag0 != "" ? URI_TAG(node, tag0) : node
				} else if ((type == "https" || type == "http") && list0[i].indexOf(",") == -1) {
					if (listi.indexOf("@") != -1) {
						node = HPS2QX(list0[i], Ptfo, Pcert0, PTls13) node = tag0 != "" ? URI_TAG(node, tag0) : node
					} else {
						var listh = Base64.decode(listi.split(type + "://")[1].split("#")[0].split("?")[0]) listh = list0[i].replace(listi.split(type + "://")[1].split("#")[0].split("?")[0], listh) node = HPS2QX(listh, Ptfo, Pcert0, PTls13) node = tag0 != "" ? URI_TAG(node, tag0) : node
					}
			...
			node = TLS_Check(node) if (node instanceof Array) {
				for (var j in node) {
					node[j] = Pudp != 0 ? XUDP(node[j], Pudp) : node[j] node[j] = Ptfo != 0 ? XTFO(node[j], Ptfo) : node[j] QXlist.push(node[j])
				}
			} else if (node != "" && node) {
				node = Pudp != 0 ? XUDP(node, Pudp) : node node = Ptfo != 0 ? XTFO(node, Ptfo) : node QXlist.push(node)
			}
		}
	}
	if (failedList.length > 0 && Pntf0 != 0) {
		$notify(`⚠️有$ {
			failedList.length
		}条数据解析失败, 已忽略`, "出错内容👇", failedList.join("\n"))
	}
	return QXlist
}
...
function AeadVmess(cnt, aeadp) {
	...
	if (/^vmess\s*\=/.test(cnt)) {
		if (/aead\s*\=/.test(cnt)) {
			...
			cnts = cnt.split(",") cnts.push(paead) cnt = cnts.join(", ")
		}
	}
	return cnt
}
...
function TLS_Check(cnt) {
	cnt = cnt.indexOf("tls-cert-sha256") != -1 || cnt.indexOf("tls-pubkey-sha256") != -1 ? cnt.replace(/tls-verification\s*\=\s*false.*?\,/, "tls-verification=true,") : cnt
	return cnt
}

function QX_TLS(cnt, Pcert0, PTls13) {
	cnt = cnt.replace(/tag\s*\=/gm, "tag=") var cert0 = Pcert0 == 1 ? "tls-verification=true, " : "tls-verification=false, "
...
function SIP2QuanX(cnt) {
	cnt = JSON.parse(cnt) ll = cnt.length nodes = []
	for (i = 0; i < ll; i++) {
		node = "shadowsocks= "
		cnti = cnt[i] ip = cnti.server + ":" + cnti.server_port mtd = "method=" + cnti.method pwd = "password=" + cnti.password obfs = cnti.plugin_opts ? cnti.plugin_opts.replace(";", ", ") : ""
		tag = "tag=" + cnti.remarks node = node + [ip, pwd, mtd, obfs, tag].filter(Boolean).join(", ") nodes.push(node)
	}
	return nodes.join("\n")
}
...
function HPS2QX(subs, Ptfo, Pcert0, PTls13) {
	...
	var server = subs.replace("https://", "").replace("http://", "").trim() var nss = []
	if (server != "") {
		if (server.indexOf("@") != -1) {
			...
			var ipport = server.split("#")[0].indexOf(":") == -1 ? "http=" + Base64.decode(server.split("#")[0].split("?")[0]) : "http=" + server.split("#")[0].split("?")[0]
		}
		var tag = "tag=" + decodeURIComponent(server.split("#")[1]);
		var tls = type == "https" ? "over-tls=true" : "";
		var thost = subs.indexOf("peer=") != -1 ? "tls-host=" + subs.split("peer=")[1].split("#")[0].split("&")[0] : ""
...
function VQ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
	var server = String(Base64.decode(subs.replace("vmess://", "").trim()).split("\u0000")[0]) var node = ""
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
	...
	if (subs.indexOf("#") == -1) {
		tag = /remarks*=/.test(subs) ? "tag=" + decodeURIComponent(subs.split(/remarks*=/)[1].split("&")[0]) : "tag=" + server.split("@")[1]
	} else {
		tag = "tag=" + subs.split("#")[1]
	}
	var tfo = subs.indexOf("tfo=1") != -1 ? "fast-open=true, " : "fast-open=false, "
	var udp = Pudp == 1 ? "udp-relay=false, " : "udp-relay=false, ";
	var pdrop = 0 node = ip + pwd + tfo + udp
	...
	if (obfs == "none") {
		...
		var ouri = subs.indexOf("&path=") != -1 ? decodeURIComponent(subs.split("&path=")[1].split("&")[0]) : "/"
...
function S5R2QX(cnt, tlsp = "false") {
	var listh = Base64.decode(cnt.split("socks://")[1].split("#")[0].split("?")[0]) server = listh + "#" + cnt.split("?")[1]
	var nss = []
	if (server != "") {
...
function Fobfs(jsonl, Pcert0, PTls13) {
	...
	if (jsonl.net == "ws" && jsonl.tls == "tls") {
		...
		uri0 = uri0.indexOf("uri=/") != -1 ? uri0 : uri0.replace("uri=", "uri=/") host0 = jsonl.host && jsonl.host != "" ? "obfs-host=" + jsonl.host + ", " : "";
		obfsi.push(obfs0 + host0 + uri0) return obfsi.join(", ")
		...
		uri0 = uri0.indexOf("uri=/") != -1 ? uri0 : uri0.replace("uri=", "uri=/") host0 = jsonl.host && jsonl.host != "" ? ", obfs-host=" + jsonl.host : "";
		obfsi.push(obfs0 + host0) return obfsi.join(", ")
		...
		Perror = 1 $notify("⚠️ Quantumult X 不支持该类型节点", "vmess + " + jsonl.net, JSON.stringify(jsonl)) return "NOT-SUPPORTTED"
...
function Regex(content) {
	var Preg0 = RegExp(Preg, "i") cnt = content
	if (Preg0.test(cnt)) {
		return content
	}
}
...
function RegexOut(content) {
	var Preg0 = RegExp(Pregout, "i") cnt = content
	if (!Preg0.test(cnt)) {
		return content
	} else {
		RegoutList.push(cnt)
...
function Scheck(content, param) {
	name = content.replace(/tag\s*\=/g, "tag=").split("tag=")[1].toUpperCase() param = param ? param.map(Dot2) : param
	if (param) {
		var flag = 0;
...
function Filter(servers, Pin, Pout) {
	...
	var Delist = [];
	var Nname = [];
	servers = servers.filter(Boolean) for (var i = 0; i < servers.length; i++) {
		if (Scheck(servers[i], Pin) != 0 && Scheck(servers[i], Pout) != 1) {
			Nlist.push(servers[i]) Nname.push(servers[i].replace(/ /g, "").split("tag=")[1])
		} else {
...
function SSR2QX(subs, Pudp, Ptfo) {
	...
	if (cnt.split(":").length <= 8) {
		...
		ssrp = cnt.split(":")[2] != "origin" ? "ssr-protocol=" + cnt.split(":")[2] : "";
		...
		tag = "tag=" + (Base64.decode(cnt.split("remarks=")[1].split("&")[0].replace(/-/g, "+").replace(/_/g, "/"))).split("\u0000")[0] pudp = Pudp == 1 ? "udp-relay=true" : "udp-relay=false";
		ptfo = Ptfo == 1 ? "fast-open=true" : "fast-open=false";
		nssr.push(type + ip, pwd, mtd, obfs, obfshost, oparam, ssrp, pudp, ptfo, tag) QX = nssr.filter(Boolean).join(", ")
	} else {
		QX = ""
	}
	return QX
...
function TJ2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
	var ntrojan = []
	var cnt = subs.split("trojan://")[1] type = "trojan=";
	...
	thost = cnt.indexOf("peer=") != -1 ? "tls-host=" + cnt.split("peer=")[1].split(/&|#/)[0] : thost ptls13 = PTls13 == 1 ? "tls13=true" : "tls13=false"
	puri = ""
	if (Pcert0 == 0) {
	...
	if (cnt.indexOf("&plugin=obfs-local") != -1) {
		obfs = cnt.indexOf("obfs=websocket") != -1 ? "obfs=wss" : obfs thost = cnt.indexOf("obfs-host=") == -1 ? thost : "obfs-host=" + decodeURIComponent(cnt.split("obfs-host=")[1].split(";")[0].split("#")[0]) puri = cnt.indexOf("obfs-uri=") == -1 ? puri : "obfs-uri=" + decodeURIComponent(cnt.split("obfs-uri=")[1].split(";")[0].split("#")[0])
	} else if (cnt.indexOf("&type=ws") != -1 || cnt.indexOf("?type=ws") != -1) {
		obfs = cnt.indexOf("security=tls") != -1 ? "obfs=wss" : obfs thost = cnt.indexOf("&host=") == -1 ? thost : "obfs-host=" + decodeURIComponent(cnt.split("&host=")[1].split("&")[0].split("#")[0]) puri = cnt.indexOf("&path=") == -1 ? puri : "obfs-uri=" + decodeURIComponent(cnt.split("&path=")[1].split("&")[0].split("#")[0])
	}
	ntrojan.push(type + ip, pwd, obfs, pcert, thost, puri, pudp, ptfo, tag) QX = ntrojan.filter(Boolean).join(", ");
	return QX
}
...
function SS2QX(subs, Pudp, Ptfo) {
	...
	if (cnt.split(":").length <= 6) {
		...
		mtd = "method=" + pwdmtd[0];
		pwdmtd.splice(0, 1) pwd = "password=" + pwdmtd.reduce(joinx);
		if (cntt.indexOf("v2ray-plugin") == -1 && cntt.indexOf("plugin=v2ray") == -1) {
			...
			cnt1 = JSON.parse(cnt1) obfs = cnt1.tls ? ", obfs=wss" : ", obfs=ws"
			obfshost = cnt1.host ? ", obfs-host=" + cnt1.host + ", tls-verification=false" : ""
		} else if (cntt.indexOf("v2ray-plugin") != -1) {
			cnt1 = decodeURIComponent(cntt.split("v2ray-plugin")[1]) obfs = cnt1.indexOf("tls") != -1 ? ", obfs=wss" : ", obfs=ws"
			...
			cnt1 = decodeURIComponent(cntt.split("plugin=v2ray")[1]) obfs = cnt1.indexOf("tls") != -1 ? ", obfs=wss" : ", obfs=ws"
			obfshost = cnt1.indexOf("host=") != -1 ? ", obfs-host=" + cnt1.split("host=")[1].split(";")[0].split("#")[0].trim() : ""
			obfshost = obfshost != "obfs-host=" ? obfshost : ""
		}
		tag = decodeURIComponent(cnt.split("#")[1]) != "undefined" ? "tag=" + decodeURIComponent(cnt.split("#")[1]) : "tag=" + ip pudp = Pudp == 1 ? "udp-relay=true" : "udp-relay=false";
		ptfo = Ptfo == 1 ? "fast-open=true" : "fast-open=false";
		nssr.push(type + ip, pwd, mtd + obfs + obfshost, pudp, ptfo, tag) QX = nssr.join(", ") return QX
	}
}
...
function SSD2QX(subs, Pudp, Ptfo) {
	var j = 0
	var QX = []
	var cnt = JSON.parse(Base64.decode(subs.split("ssd://")[1])) var type = "shadowsocks=";
...
function QXFix(cntf) {
	var cnti = cntf.replace(/\s*tag\s*\=/g, "tag=").replace("chacha20-poly", "chacha20-ietf-poly") try {
		var hd = cnti.split(",tag=")[0]
		var tag = "tag=" + cnti.split(",tag=")[1].split(",")[0].trim() var tail = cnti.split(tag + ",") cnti = tail.length <= 1 ? cnti : String(hd + "," + tail[1].split("\r")[0] + "," + tag) cntis = cnti.split(",").filter(Boolean).map(item = > item.trim()) tagfix = ""
		cntii = ""
		for (i in cntis) {
...
function isQuanX(content) {
	...
	for (var i = 0; i < cnts.length; i++) {
		...
		if (cnti.indexOf("=") != -1 && cnti.indexOf("tag") != -1) {
			var cnt = cnti.split("=")[0].trim() if (cnt == "http" || cnt == "shadowsocks" || cnt == "trojan" || cnt == "vmess" || cnt == "socks5") {
...
function isQuanXRewrite(content) {
	...
	for (var i = 0; i < cnt.length; i++) {
		if (cnt[i]) {
			var cnti = cnt[i].trim() const RuleCheck = (item) = > cnti.toLowerCase().indexOf(item) != -1;
			if (cnti.indexOf("pattern") != -1 && cnti.indexOf("type") != -1 || cnti.indexOf("http-r") != -1) {
				...
				cnti = cnti.replace("^http", "http") cnti = cnti.split(" ")[1] == "url" ? cnti : ""
...
function ToTag(elem1, elem2) {
	var tag1 = elem1.split("tag")[1].split("=")[1].trim() var tag2 = elem2.split("tag")[1].split("=")[1].trim() res = tag1 > tag2 ? 1 : -1
	return res
}
...
function Sort_KWD(cnt, strs) {
	strlist = strs.indexOf("<") != -1 ? strs.split("<") : strs.split(">") regj = strlist.map(item = > RegExp(item, "i")) dir = strs.indexOf("<") != -1 ? -1 : 1
	...
	arr = PsortX == -1 ? arr.map(item = > item.sort(ToTagR)) : arr arr = PsortX == 1 ? arr.map(item = > item.sort(ToTag)) : arr newarr = MixArr(arr, dir) return newarr
}
...
function MixArr(cnt, dir) {
	...
	cnt0 = dir == 1 ? cnt0.concat(cnt[cnt.length - 1].sort(ToTag)) : (cnt[cnt.length - 1].sort(ToTagR)).concat(cnt0) return cnt0
}

function DelReg(content) {
	delreg = RegExp(delreg, "gmi") content = content.replace(/tag\s*\=\s*/, "tag=") cnt0 = content.split("tag=")[0] cnt1 = content.split("tag=")[1].split(",")[0] cnt2 = content.split("tag=")[1].split(",").length == 1 ? "" : content.split(cnt1)[1] cnt = cnt0 + "tag=" + cnt1.replace(delreg, "") + cnt2
	return cnt
}
...
function Rename(str) {
	...
	if (server.indexOf("tag=") != -1) {
		hd = server.split("tag=")[0] name = server.split("tag=")[1].split(",")[0].trim() tail = server.split("tag=")[1].split(",").length <= 1 ? "" : server.split("tag=")[1].split(name)[1]
		for (var i = 0; i < Prn.length; i++) {
			...
			if (oname && nname) {
				var rn = escapeRegExp(oname) name = name.replace(new RegExp(rn, "gmi"), nname)
			} else if (oname && nname == "") {
				var nemoji = emoji_del(name) if ((Pemoji == 1 || Pemoji == 2) && Prname) {
				...
				hh = Dot2(oname.slice(0, oname.length - 2)).split(".") for (j = 0; j < hh.length; j++) {
					var nn = escapeRegExp(ToDot(hh[j])) var del = new RegExp(nn, "gmi");
...
function get_emoji(emojip, sname) {
	...
	str1 = JSON.stringify(Lmoji) aa = JSON.parse(str1) bb = JSON.parse(str1.replace(/🇹🇼/g, " 🇨🇳")) var cnt = emojip == 1 ? aa : bb;
	var flag = 0;
	for (var key in cnt) {
		dd = cnt[key]
		for (i in dd) {
			if (sname.indexOf(dd[i]) != -1) {
				flag = 1;
				nname = key + " " + sname.replace(/[\uD83C][\uDDE6-\uDDFF][\uD83C][\uDDE6-\uDDFF]/g, "").trim();
				return [nname, key]
			}
		}
	}
...
function emoji_handle(servers, Pemoji) {
	...
	for (var i = 0; i < ser0.length; i++) {
		if (ser0[i].indexOf("tag=") != -1) {
			...
			var nserver = hd + "tag=" + nname.replace("  ", " ").trim() nlist.push(nserver)
		}
	}
	return nlist
}
...
function Surge2QX(conf) {
	var QXlist = conf.split("\n").map(isSurge).filter(Boolean) var Nlist = []
	var node = ""
	for (var i = 0; i < QXlist.length; i++) {
		...
		node = Pudp0 != 0 ? XUDP(node, Pudp0) : node node = Ptfo0 != 0 ? XTFO(node, Ptfo0) : node
		if (cnt.indexOf("test-url") != -1) {
			var checkurl = ", server_check_url" + cnt.split("test-url")[1].split(",")[0] node = node.replace(/\,(\s)*tag/, checkurl + ", tag")
		}
		Nlist.push(node)
	}
	return (Nlist)
...
function isSurge(content) {
	if (content.indexOf("=") != -1) {
		cnt = content.split("=")[1].split(",")[0].trim() if (cnt == "http" || cnt == "ss" || cnt == "trojan" || cnt == "vmess" || cnt == "custom" || cnt == "https" || cnt == "socks5" || cnt == "socks5-tls") {
			return content
		}
	}
}
...
function paraCheck(content, para) {
	content = content.replace(/ /g, "") if (content.indexOf(para + "=") == -1) {
		return "false"
	} else {
...
function Loon2QX(cnt) {
	var type = cnt.split("=")[1].split(",")[0].trim() var node = ""
	if (type == "Shadowsocks") {
		node = LoonSS2QX(cnt)
...
function LoonSS2QX(cnt) {
	...
	var ip = [cnt.split(",")[1].trim(), cnt.split(",")[2].trim()].join(":") var mtd = "method=" + cnt.split(",")[3].trim() var pwd = "password=" + cnt.split(",")[4].trim().split("\"")[1]
	var obfs = cnt.split(",").length == 7 ? ", " + ["obfs=" + cnt.split(",")[5].trim(), "obfs-host=" + cnt.split(",")[6].trim()].join(",") : ""
	var tag = ", tag=" + cnt.split("=")[0].trim() node = node + [ip, mtd, pwd].join(", ") + obfs + tag
	return node
}
...
function LoonSSR2QX(cnt) {
	var node = "shadowsocks="
	var ip = [cnt.split(",")[1].trim(), cnt.split(",")[2].trim()].join(":") var mtd = "method=" + cnt.split(",")[3].trim() var pwd = "password=" + cnt.split(",")[4].trim().split("\"")[1]
	var ssrp = "ssr-protocol=" + cnt.split(",")[5].trim() var ssrpara = "ssr-protocol-param=" + cnt.split(",")[6].replace(/\{|\}/g, "").trim() var obfs = "obfs=" + cnt.split(",")[7].trim() var obfshost = "obfs-host=" + cnt.split(",")[8].replace(/\{|\}/g, "").trim() var tag = ", tag=" + cnt.split("=")[0].trim() node = node + [ip, mtd, pwd, ssrp, ssrpara, obfs, obfshost].join(", ") + tag
	return node
}

function YAMLFix(cnt) {
	cnt = cnt.replace(/\[/g, "yaml@bug1").replace(/\\r/g, "").replace(/\*/g, "yaml@bug2") if (cnt.indexOf("{") != -1 && /\{\s*\"*(name|type|server)/.test(cnt)) {
	...
	cnt = cnt.replace(/\n\s*\-\s*\n.*name/g, "\n  - name").replace(/\$|\`/g, "").split("proxy-providers:")[0].split("proxy-groups:")[0].replace(/\"(name|type|server|port|cipher|password|uuid|alterId|udp)(\"*)/g, "$1") if (Pdbg == 1) {
		$notify("part-fix0:", "", "part-fix0:\nproxies:\n" + cnt.split("proxies:")[1])
	}
	cnt = cnt.replace(/\n\s{2}([a-zA-Z]+.*\:)/g, "\n    $1").replace(/\n(\-.*)/g, "\n  $1") if (Pdbg == 1) {
		$notify("part-fix1:", "", "part-fix1:\nproxies:\n" + cnt.split("proxies:")[1])
	}
	cnt = cnt.replace(/name\:(.*?)\:(.*?)\n/gmi, "name:$1冒号$2\n").replace(/\s{6}Host\:/g, "      Host:") items = cnt.split("\n").map(yamlcheck) cnt = items.join("\n") cnt = cnt.indexOf("proxies:") != -1 && /\n\s{4}server/.test(cnt) ? cnt.replace(/\n\s{4}(headers|path)/g, "\n      $1").replace(/\n\s{6}Host/g, "\n        Host").replace(/\t/g, "") : cnt cnt = cnt.indexOf("proxies:") == -1 ? "proxies:\n" + cnt : "proxies:" + cnt.split("proxies:")[1] console.log("after-fix\n" + cnt) if (Pdbg == 1) {
...
function Clash2QX(cnt) {
	const yaml = new YAML() if (Pdbg == 1) {
		$notify(" Before YAML Parse", "content", cnt)
	}
	var aa = JSON.stringify(yaml.parse(YAMLFix(cnt))).replace(/yaml@bug𝟙/g, "[").replace(/冒号/gmi, ":").replace(/yaml@bug𝟚/g, "*") for (var i = 0; i < 10; i++) {
	...
	for (i = 0; i < nl; i++) {
		try {
			...
			node = Pudp0 != 0 ? XUDP(node, Pudp0) : node node = Ptfo0 != 0 ? XTFO(node, Ptfo0) : node nodelist.push(node)
		} catch (e) {
			$notify(`⚠️该节点解析错误, 暂时已忽略处理`, `可点击通知并发送链接反馈至bot`, JSON.stringify(node), bug_link) $notify(`⚠️错误内容如下`, `可复制错误内容到反馈bot`, JSON.stringify(node) + "\n\n" + e)
		}
	}
...
function CSS2QX(cnt) {
	tag = "tag=" + cnt.name.replace(/\\U.+?\s{1}/gi, "") ipt = cnt.server + ":" + cnt.port pwd = "password=" + cnt.password mtd = "method=" + cnt.cipher udp = cnt.udp ? "udp-relay=true" : "udp-relay=false"
	...
	node = "shadowsocks=" + [ipt, pwd, mtd, udp, uot, tfo, obfs, ohost, ouri, cert, tag].filter(Boolean).join(", ") return node
}

function CSSR2QX(cnt) {
	tag = "tag=" + cnt.name.replace(/\\U.+?\s{1}/gi, "") ipt = cnt.server + ":" + cnt.port pwd = "password=" + cnt.password mtd = "method=" + cnt.cipher udp = cnt.udp ? "udp-relay=true" : "udp-relay=false"
	tfo = cnt.tfo ? "fast-open=true" : "fast-open=false"
	prot = "ssr-protocol=" + cnt.protocol ohost = ""
	ppara = ""
	...
	if (typeof(cnt["protocol-param"]) == "string") {
		...
		console.log(typeof(cnt["protocol-param"])) ppara = "ssr-protocol-param=" + JSON.stringify(cnt["protocol-param"]).replace(/{|}|\s|"/g, "")
	}
	obfs = "obfs=" + cnt.obfs
	if (cnt["obfs-param"]) {
	...
	node = "shadowsocks=" + [ipt, pwd, mtd, udp, tfo, prot, ppara, obfs, ohost, tag].filter(Boolean).join(", ") return node
}

function CV2QX(cnt) {
	tag = "tag=" + cnt.name.replace(/\\U.+?\s{1}/gi, " ") ipt = cnt.server + ":" + cnt.port pwd = "password=" + cnt.uuid mtd = "method=" + "aes-128-gcm"
	...
	console.log(obfs) const phost = getValue(() = > cnt["ws-opts"]["headers"]["Host"]) ohost = cnt["ws-headers"] ? "obfs-host=" + cnt["ws-headers"]["Host"] : ""
	ohost = phost ? "obfs-host=" + phost : ohost ohost = cnt["servername"] ? "obfs-host=" + cnt["servername"] : ohost console.log(ohost) ouri = cnt["ws-path"] ? "obfs-uri=" + cnt["ws-path"] : ""
	ouri = cnt["ws-opts"] ? "obfs-uri=" + cnt["ws-opts"]["path"] : ouri cert = cnt["skip-cert-verify"] && cnt.tls ? "tls-verification=false" : ""
	caead = cnt["alterId"] && cnt["alterId"] != 0 ? "aead=false" : ""
	console.log(caead) if (Pcert0 == 1 && cnt.tls) {
		cert = "tls-verification=true"
	} else if (Pcert0 != 1 && cnt.tls) {
		cert = "tls-verification=false"
	}
	node = "vmess=" + [ipt, pwd, mtd, udp, tfo, obfs, ohost, ouri, cert, caead, tag].filter(Boolean).join(", ") return node
}

function CT2QX(cnt) {
	tag = "tag=" + cnt.name.replace(/\\U.+?\s{1}/gi, " ") ipt = cnt.server + ":" + cnt.port pwd = "password=" + cnt.password otls = "over-tls=true"
	opath = ""
	ohost = ""
	...
	node = "trojan=" + [ipt, pwd, otls, opath, ohost, cert, tls13, udp, tfo, tag].filter(Boolean).join(", ") return node
}

function CH2QX(cnt) {
	tag = "tag=" + cnt.name.replace(/\\U.+?\s{1}/gi, " ") ipt = cnt.server + ":" + cnt.port uname = cnt.username ? "username=" + cnt.username : ""
	...
	node = "http=" + [ipt, uname, pwd, tls, cert, tag].filter(Boolean).join(", ") return node
}
...
function CS52QX(cnt) {
	...
	node = "socks5=" + [ipt, uname, pwd, tls, cert, tag].filter(Boolean).join(", ") return node
}
...
function UOT(cnt) {
	cnts = cnt.replace(/\s*/g, "") if (/^shadowsocks=/.test(cnts)) {
...
function YAML() {
	...
	function parser(str) {
		...
		for (var i = 0, len = lines.length; i < len; ++i) {
			...
			if (m = regLevel.exec(line)) {
				level = m[1].length
			} else level = 0; if (level > curLevel) {
	...
	this.parse = function eval0(str) {
		...
		var pre = preProcess(str) var doc = parser(pre);
